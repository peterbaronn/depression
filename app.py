...
st.write(
    "This app predicts student depression risk using "
...
work_study_hours = st.slider(
    "Work/Study Hours",
    0.0, 12.0, 6.0, step=1.0
)

financial_stress = st.slider(
    "Financial Stress",
    1.0, 5.0, 3.0, step=1.0
)
...
def preprocess_input():
    ...
    input_data["Sleep Duration"] = input_data["Sleep Duration"].map({
        "Others": 0,
        "Less than 5 hours": 1,
        "5-6 hours": 2,
        "7-8 hours": 3,
    ...
    input_data["Dietary Habits"] = input_data["Dietary Habits"].map({
        "Healthy": 0,
        "Moderate": 1,
        "Unhealthy": 2,
        "Others": 3
    ...
    input_data["Have you ever had suicidal thoughts ?"] = input_data[
        "Have you ever had suicidal thoughts ?"
    ].map({
        "No": 0,
        "Yes": 1
    ...
    input_data["Family History of Mental Illness"] = input_data[
        "Family History of Mental Illness"
    ].map({
        "No": 0,
        "Yes": 1
...
if st.button("Predict Depression Risk"):
    ...
    if prediction == 1:
        st.error("Result: Depressed Risk Detected")
    else:
        st.success("Result: Not Depressed")

    st.write(f"Depression Probability: **{probability:.2%}**")
